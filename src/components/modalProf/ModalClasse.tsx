import { classe } from "@/hooks/api/Classe/type";
import { useAddClasse } from "@/hooks/api/Classe/useAddClasse";
...
const ModalClasse = ({ isOpenClasse, closeModalClasse }: any) => {
  const { register, handleSubmit } = useForm<Inputs>();
  const { mutate } = useAddClasse();

  const onSubmit: SubmitHandler<Inputs> = (data) => {
    mutate(data, { onSettled: () => closeModalClasse() });
  };
  return (
    <>
      <Transition appear show={isOpenClasse} as={Fragment}>
        <Dialog as="div" className="relative z-10" onClose={closeModalClasse}>
          <Transition.Child
            as={Fragment}
