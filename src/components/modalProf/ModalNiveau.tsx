import { useAddNiveau } from "@/hooks/api/Niveau/useAddNiveau";
...
const ModalNiveau = ({ isOpenNiveau, closeModalNiveau }: any) => {
  const { register, handleSubmit } = useForm<Inputs>();
  const { mutate } = useAddNiveau();

  const onSubmit: SubmitHandler<Inputs> = (data) => {
    mutate(data, { onSettled: () => closeModalNiveau() });
  };
  return (
    <>
      <Transition appear show={isOpenNiveau} as={Fragment}>
        <Dialog as="div" className="relative z-10" onClose={closeModalNiveau}>
          <Transition.Child
            as={Fragment}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Nombre d'eleve"
                        {...register("nombreEleve")}
                        required
                      />
                    </div>
